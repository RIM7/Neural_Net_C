...
void take_all_inputs(){
  ...
  ip_samples[0][1] = 0.10;
    ...
    op_samples[i] = calloc(nodes_in_layer[no_of_layers-1], sizeof(int *));
	op_samples[0][0] = 0.01;
  op_samples[0][1] = 0.99;

	records = calloc(no_of_layers-1, sizeof(int*));
	for(int i=0; i<no_of_layers-1; i++){
		records[i] = calloc(nodes_in_layer[i], sizeof(int*)); for(int j=0; j<nodes_in_layer[i]; j++) records[i][j] = calloc(nodes_in_layer[i+1], sizeof(int*)); 
	}
...
void print_all_inputs(){
	printf("\ninput samples:\n");
	for(int i=0; i<samples; i++){ for(int j=0; j<nodes_in_layer[0]; j++) printf("%lf ", ip_samples[i][j]); printf("\n");}

	printf("\noutput samples:\n"); for(int i=0; i<samples; i++){ for(int j=0; j<nodes_in_layer[no_of_layers-1]; j++) printf("%lf ", op_samples[i][j]); printf("\n");}

	for(int i=0; i<no_of_layers-1; i++) { 
		printf("\nWeights at layer %d -> %d:\n", i, i+1); 
		for(int j=0; j<nodes_in_layer[i]; j++) {  for(int k=0; k<nodes_in_layer[i+1]; k++) printf("%lf ", records[i][j][k]); printf("\n"); } printf("\n\n");
	}
